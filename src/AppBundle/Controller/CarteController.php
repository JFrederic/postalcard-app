...
class CarteController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            if ($data->getType() == 'Carte Anniversaire') {
              $carte->setImage('images\carte-anniversaire.jpg');
            }
            elseif ($data->getType() == 'Carte Noël') {
              $carte->setImage('images\noel2007.jpg');
            }
            elseif ($data->getType() == 'Carte Nouvel An') {
              $carte->setImage('images\happy_new_year.jpg');
            }
